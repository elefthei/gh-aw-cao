...
describe('gh-aw logs adapter', () => {
  ...
  it('normalizes retained verifier comparisons without deriving business state', () => {
    const run = {
      schema_version: 2,
      kind: 'token_efficiency_run_context',
      run: {
    ...
    const batch = normalize(adaptCachedGhAwJsonl(
      `${JSON.stringify(run)}\n${JSON.stringify(comparison)}\n`
    ).observations);
    expect(relationshipErrors(batch)).toEqual([]);
    expect(batch.events.find((event) => event.comparisonId === 'token-comparison:1'))
      .toMatchObject({
  ...
  it('uses retained operational-value artifacts for canonical grader evidence', () => {
    const run = {
      schema_version: 2,
      kind: 'token_efficiency_run_context',
      run: {
        ...
        workflow_path: '.github/workflows/review.md',
        status: 'completed',
        conclusion: 'success',
        created_at: '2026-10-02T00:00:00Z',
        updated_at: '2026-10-02T00:02:00Z',
    ...
    const grader = {
      ...
      observation: {
        ...
        result: {
          ...
          value: 1,
          direction: 'maximize',
          unit: 'accepted outcome',
          implementation: {
            digest: 'a'.repeat(64)
          },
          observation: {
            mature: true,
            evidenceAt: '2026-10-03T00:00:00Z'
          }
        }
      }
    };
    ...
    expect(relationshipErrors(batch)).toEqual([]);
    expect(batch.events.filter((event) => event.type === 'workflow_run_grader')).toEqual([
      expect.objectContaining({
        grader: 'operational-value',
        status: 'pass',
        implementation: { digest: 'a'.repeat(64) },
        observation: {
          mature: true,
          evidenceAt: '2026-10-03T00:00:00Z'
        }
