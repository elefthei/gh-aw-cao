...
- `optimization-token-efficiency-verifier`: consumes exactly one authoritative applied intervention with a frozen baseline and matured optimized window, and publishes a structured claim for deterministic Activity comparison and append-only verified, regressed, or inconclusive lifecycle evidence. It is always review-only and never writes a target repository.

Dispatch stays repository-scoped: one dispatch per selected repository and eligible worker. Dispatch `optimization-token-optimizer` only with all of its frozen assignment inputs, including the exact `evaluator_digest` from the target workflow's authoritative schema-version-4 operational-value result; do not ask it to discover an opportunity and do not substitute the optimizer's evaluator. Dispatch `optimization-token-efficiency-verifier` only for one latest authoritative lifecycle observation whose disposition is `applied` and whose implementation is complete. Supply only `target_repo`, `opportunity_id`, `intervention_id`, and an `evidence_cutoff` from the complete Activity generation. The cutoff is a temporal selector and must not be later than dispatch; the verifier derives maturity, windows, experiment, variants, evaluator, target Run attempts, operational-value outcomes, and overhead from retained authoritative evidence. Do not supply Run lists, measurements, outcome lists, AIC, or evaluator contracts. Do not dispatch proposed, unapplied, rejected, failed-start, superseded, outdated, duplicate, unmatured, stale, ambiguous, or mixed-grain evidence. The ambient-context workers apply their existing 10 percent gain gate before publishing and return a `noop` when the estimated reduction in always-loaded context is smaller.
